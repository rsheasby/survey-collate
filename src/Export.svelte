<script lang="ts">
    ...
    function exportCsv() {
        ...
        questions.forEach((q, i) => {
            let answers = q.answers.slice(0, exportLimit).filter(a => a.points >= exportThreshold).map(a => `${a.points} - ${a.text}`);
            exportResults.push([q.text, ...answers]);
        });
        let outputBlob = new Blob([Papa.unparse(exportResults, {
            quotes: true,
            header: false,
        })]);
    ...
    function exportJson() {
        let exportResults = questions.map(q => ({
            text: q.text,
            answers: q.answers.slice(0, exportLimit).filter(a => a.points >= exportThreshold).map(a => ({
                text: a.text,
                points: a.points
            }))
        }));
